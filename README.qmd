...
subtitle: "Jan. 14th, 2026, Joshua Holt and Sam Larralde"
format:
  pdf: default
  gfm: default
---
...
## \# 1.1) It doesn't look like there is a clear relationship between these variables. The data seems to be clustered around 2 central points one having more depth and being more dense.
...
# 1.2) This time, there seems to linear positive relationship between bill length and bill depth. This is made more obvious with the clear distinction between species, so now we can tell that the data follows the same trends for all three species. This was not present before, making the message from the data unclear.
...
Using your new color skills, modify the `scale_color_*` code below to include a new color selection! \# 1.3) Changed color group drom Dark2 to Set3
...
# 1.4) Replaced \* with NA so I ended with \`limits = c(0,NA)
...
# 1.5) We added: shape = "Species", fill = "Species", color = "Species"
...
# 1.6) "plain", "italic", "bold", "bold.italic"
...
# 2.1) A community member who regularly attends townhalls. Is very involved with local events.
...
ab_la <- read.csv("abalone_landings.csv")
ggplot(ab_la, aes(x = Year, y = Abalone_Landings_lbs)) +
  geom_line(aes(color= Abalone_Species)) +
  geom_point(aes(color= Abalone_Species))
ggsave("Abalone_Landings_per_Year.png")
```
...
# 2.3) We would like to revise y-axis name, add title, revise legend name, set color palette for species, and take points off
...
ab_la <- read.csv("abalone_landings.csv")
ggplot(ab_la, aes(x = Year, y = Abalone_Landings_lbs, color = Abalone_Species)) +
  geom_line()+
  scale_color_manual(values = c("Black" ="black",
                                ...
                                "Pink"="pink"))+
  labs(x = "Year",
     y = "Abalone Landings in Lbs",
     title = "Abalone Landing Decline Over Time",
     Abalone_Species = "Abalone Species",
     color = "Abalone Species") + scale_x_continuous(limits = c(1973,1997)) + theme_classic()
ggsave("Abalone_Landings_per_Year.png")
```
...
# 2.5) We hope that concerned community members recognize the degree of biomass loss in abalone over the years. We also hope that they are able to discern how well the different species fared.
